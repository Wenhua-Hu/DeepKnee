...
############### to open heatmap
img_path = r"C:\Users\whu\Desktop\DSP_project\UVA21_DSP_QUIN\apps\static\assets\images\knee_gradcam\9003430L_gradcam.png"
heatmap = Image.open(img_path)
heatmap_ass_array = np.asarray(heatmap)

if np.max(heatmap_ass_array) <= 1:
    heatmap_ass_array = heatmap_ass_array * 255

print(heatmap_ass_array)
...
#Define paramters = 
cmap=matplotlib.colors.LinearSegmentedColormap.from_list('gr',['r', 'g'], N=256) 
min_intensity = 140
min_size = 625
min_perc = 0.03
intensity_based_size = False
accuracy = False


group_class = Groupmatrix(heatmap_ass_array, min_intensity)
group_class.find_groups()
out_groups = group_class.return_groups(min_size,min_perc)

# Draw groups
hs = HeatmapSquares(heatmap, out_groups)
output_image = hs.draw_all_groups(intensity_based_size, accuracy)